...
#validate(config, schema=".OB/schemas/config.schema.yaml")

caseinfo = pd.read_csv(config["caseinfo"]).set_index(["case","sample","unit"], drop=False)
pairedinfo = pd.read_csv(config["pairedinfo"],index_col=0)
#sampleinfo= pd.read_csv(config["caseinfo"]).set_index(["case","sample"], drop=False)
#validate(samples, schema="../schemas/samples.schema.yaml")

#units = pd.read_table(config["units"], dtype=str).set_index(["sample", "unit"], drop=False)
#pairedinfo.index = pairedinfo.set_levels([i.astype(str) for i in pairedinfo.index.levels])  # enforce str in index
...
def get_paired_bam(wildcards):
    T=pairedinfo.loc[wildcards.case,"T"]
    N=pairedinfo.loc[wildcards.case,"N"]
    return {"T":T, "N": N}
